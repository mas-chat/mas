...
async function processConnected({ userId, network }) {
  ...
  const commands = [`NICK ${user.get('nick')}`, `USER ${user.get('nick')} 8 * :${user.get('name')} (MAS v2.0)`];
...
async function processDisconnected({ userId, network, reason }) {
  ...
  await networkInfo.set('state', previousState === 'idleclosing' ? 'idledisconnected' : 'disconnected');
  ...
  if (count > 3 && count < 8) {
    ...
    msg =
      'Error in connection to IRC server after multiple attempts. Waiting one hour ' +
...
async function handle401(user, msg) {
  ...
  const conversation = await conversationsService.findOrCreate1on1(user, targetUserGId, msg.network);
...
async function handle311(user, msg) {
  ...
  await addSystemMessage(user, msg.network, 'server', `--- ${nick} is [${username}@${host}] (${realName})`);
}
...
async function handle312(user, msg) {
  ...
  await addSystemMessage(user, msg.network, 'server', `--- using server ${server} [${serverInfo}]`);
}
...
async function handle317(user, msg) {
  ...
  await addSystemMessage(user, msg.network, 'server', `--- has been idle ${idleTime} seconds. Signed on ${signonTime}`);
}
...
async function handle366(user, msg) {
  ...
  if (namesHash && Object.keys(namesHash).length > 0) {
    ...
    const noActiveReporter = await redis.setnx(`irc:names_reporter:${conversation.id}`, user.id);
...
async function handleJoinReject(user, msg) {
  ...
  await addSystemMessage(
    ...
    msg.network,
    'error',
    subscription ? `Failed to join ${channel}, reason: ${reason}` : `${reason}: ${channel}`
  );
...
async function handle433(user, msg) {
  ...
  if (networkInfo.get('state') !== 'connected') {
    ...
    await addSystemMessage(user, msg.network, 'info', `ERROR: Nick ${msg.params[0]} is already taken.`);
  }
}
...
async function handleError(user, msg) {
  ...
  await addSystemMessage(user, msg.network, 'error', `Connection lost. Server reason: ${reason}`);
...
async function handleInvite(user, msg) {
  ...
  await addSystemMessage(user, msg.network, 'info', `${msg.nick} has invited you to channel ${channel}`);
}
...
async function handleKick(user, msg) {
  ...
  if (conversation) {
    await conversationsService.removeGroupMember(conversation, targetUserGId, { wasKicked: true, reason });
  }

  if (targetUserGId.equals(user.gId)) {
    // I was kicked
    await addSystemMessage(user, msg.network, 'error', `You have been kicked from ${channel}, Reason: ${reason}`);
  ...
  while (modeParams.length !== 0) {
    ...
    for (const mode of modes) {
      ...
      if (mode === 'o' && oper === '+') {
        ...
        const oldClass = await conversationsService.getMemberRole(conversation, targetUserGId);
...
async function handlePrivmsg(user, msg, command) {
  ...
  if (target.toLowerCase() === currentNick.toLowerCase()) {
    ...
    conversation = await conversationsService.findOrCreate1on1(user, sourceUserGId, msg.network);
  } else {
...
async function updateNick(user, network, oldNick, newNick) {
  ...
  if (nickUser) {
    ...
    changed = await redis.set(`irc:nick_changed_mutex:${network}:${oldNick}:${newNick}`, 1, 'ex', 20, 'nx');
  ...
  for (const conversationMember of conversationMembers) {
    ...
    if (targetUserGId.isMASUser) {
      ...
      await conversationsService.removeGroupMember(conversation, conversationMember.gId, { silent: true });

      await conversationsService.addGroupMember(
        conversation,
        await nicksService.getUserGId(newNick, network),
        conversationMember.get('role'),
        { silent: true }
      );
    }
  }
...
async function disconnectIfIdle(user, network) {
  ...
  if (onlyServer1on1Left) {
    await addSystemMessage(user, network, 'info', 'No open windows left for this network. Disconnected.');
  }
}
