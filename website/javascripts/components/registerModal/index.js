      ...
      config.auth.google || config.auth.yahoo || config.auth.cloudron ? (
        <nav className={cx('m-ext', 'level')}>
          <div className="level-left">
            <div className="level-item">Register with an existing account:</div>
          </div>
        ...
        <div className="modal-container">
          <div className={cx('m-content', 'modal-content')}>
            <section className="section">
              <Formsy onValidSubmit={this.register} onValid={this.enableButton} onInvalid={this.disableButton}>
                <h4 className="title is-4 has-text-centered">Register</h4>

                <RegisterModalInput
                  showErrorMessage={this.state.showErrors}
                  autoFocus
                  name="name"
                  validations="minLength:6"
                  validationError="Please enter at least 6 characters"
                  label="Your name"
                  autocomplete="name"
                  required
                />
                ...
                <RegisterModalInput
                  ...
                  validationError="This is not a valid nick"
                  label="Nickname"
                  autocomplete="nickname"
                  required
                />
                <RegisterModalCheckbox
                  name="tos"
                  validationError="You must agree TOS"
                  label="I agree MAS Terms of Service"
                  required
                />

                <p className={cx('m-controls', 'control')}>
                  <button type="submit" className="button is-primary" disabled={!this.state.validDetails}>
                    Register
                  </button>
                </p>
              </Formsy>
